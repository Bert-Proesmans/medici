...
pub fn start_game(machine: Machine<Wait<Start>>) -> Result<Machine<Wait<Input>>, Error> {
    // Transition into the desired state for trigger execution.
    let mut action: Machine<Action<Start>> = machine.transition(Epsilon);
    // Execute all listeners for this action.
    let mut effect: Machine<Effect<Start>> = action.pushdown(Epsilon);
